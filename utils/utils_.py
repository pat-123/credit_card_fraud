...
def get_outliers(data,continous_names, outliers_,except_ = [],drop=False,replace=False):
    ...
    for i in continous_names:
        ...
        if drop:
            ...
            data = data.reset_index().drop('index',axis=1)
        if replace:
            data.loc[(data.loc[:,i]>max)|(data.loc[:,i] <min), i]=data.loc[:,i].mean()
...
def rf_hyperparameter_tuning(iv,dv,rf_params,classific=False):
    ...
    for nt in ntrees:
        if classific:
            model = RandomForestClassifier(n_estimators=nt,random_state=42).fit(iv_train,dv_train)
        else:#by-default
    ...
    #-------- no of max features
    score_={}
    print(int(np.sqrt(iv_train.shape[1])))
    max_feat = ['log2','auto',int(np.sqrt(iv_train.shape[1])) if classific else int(np.ceil(iv_train.shape[1]/3)) ]
    for m in max_feat:
        if classific:
            model = RandomForestClassifier(n_estimators=rf_params['n_estimators'],max_features=m,random_state=42).\
...
def plot_roc(model,iv_test,dv_test):
